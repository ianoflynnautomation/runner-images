...
variable "image_os_type" {
  ...
  validation {
    ...
    error_message = "The image OS type must be either 'Linux' or 'Windows'."
  }
}
...
variable "managed_image_storage_account_type" {
  ...
  validation {
    condition     = contains(["Standard_LRS", "Premium_LRS", "Standard_ZRS"], var.managed_image_storage_account_type)
    error_message = "The managed image storage account type must be one of 'Standard_LRS', 'Premium_LRS', or 'Standard_ZRS'."
  }
}
...
variable "image_os" {
  ...
  validation {
    ...
    error_message = "The image OS must be one of 'ubuntu22' or 'ubuntu24'."
  }
}
...
variable "spot_instance" {
  ...
  validation {
    ...
    error_message = "The spot eviction policy must be 'Deallocate' or 'Delete' when spot is enabled."
  }

  validation {
    condition     = !var.spot_instance.enabled || var.spot_instance.max_price == -1 || var.spot_instance.max_price > 0
    error_message = "The spot max price must be -1 (use Azure default pricing) or greater than 0 when spot is enabled."
  }
}
...
DESCRIPTION
  ...
  validation {
    ...
    error_message = "The shared image gallery timeout must be a valid duration string (e.g., '60m', '90m', '2h')."
  }
}
...
variable "shared_gallery_image_version_replica_count" {
  ...
  validation {
    ...
    error_message = "The shared gallery image version replica count must be between 1 and 100 inclusive."
  }
}
